...
ALLOC_NOTRACE_FN
    ...
    my $refget_fn = <<"REFGET";
void *${heap_name}_refget(const char *trace) {
	#ifdef CHECK_TRACE_SIZE
	if (strlen(trace) >= TRACE_SIZE) {
		#if ALLOCPP_DEBUG
		fprintf("Refget error: `%s` is larger than maximum trace size (%d)", trace, TRACE_SIZE);
	...
	uint16_t trace_hash = ${heap_name}_hash(trace);
...
void ${heap_name}_refunget(const char *trace) {
		#ifdef CHECK_TRACE_SIZE
		if (strlen(trace) >= TRACE_SIZE) {
			#if ALLOCPP_DEBUG
			fprintf("Refget error: `%s` is larger than maximum trace size (%d)", trace, TRACE_SIZE);
...
Note: Replace HEAP with the name of your heap (which you passed to the alloc-pp directive).
	...
	void *HEAP_refget(const char *trace); 	// Increase reference count
	void HEAP_refundet(const char *trace); 	// Decreace reference count
